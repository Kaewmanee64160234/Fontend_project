...
// const getDate = (timeDB:string,type:string) =>{
//   if(type ==='Date'){
//     return timeDB.substring()
//   }else{
//     return timeDB.getFullYear()
//   }
// }
onMounted(async () => {
    ...
    <div v-for="(item, index) in employeeStore.summary_salaries" :key="index">
      ...
      >
        <div class="stat">
          ...
          >
            <!-- {{ getDate(item.ss_date?.toDateString+'','Date') }} -->
            {{ new Date(item.ss_date+'').getUTCMonth() }}
          </div>
          <br />
        ...
        <div class="stat">
          <div
            ...
            style="font-size: 29px; font-family: sans-serif"
          >
            {{ employeeStore.summary_salary.salary }} ฿
          </div>
          <br />
