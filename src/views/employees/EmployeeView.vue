        ...
        <v-card> 
            ...
            <v-card-title>
                ...
                <v-btn class="mdi mr-2  mdi-delete" style="float: right; color: white" color="red"
                    @click="deleteAllEmployees">Delete All</v-btn>
                <v-spacer> </v-spacer>
                ...
                <v-table class="text-center mt-5">
                    <thead>
                        <tr>
                            <th>
                                <v-checkbox class="d-flex pa-4" color="indigo" v-model="employeeStore.allSelected"
                                    @click="employeeStore.selectEmployeeAll"></v-checkbox>
                            </th>
                            <th></th>
                            <th>ID</th>
                            <th>Name</th>
                    ...
                    <tbody>
                        <tr  v-for="item of employeeStore.employees" :key="item.id" class="text-center">
                            <td>
                                <v-checkbox class="d-flex pa-4" color="indigo" v-model="employeeStore.selected"
                                    @click="employeeStore.selectEmployee()" :value="item.id + ''"></v-checkbox>
                            </td>
                            <td>
                                <v-avatar><v-img :src="`${url}/employees/image/${item.image}`"></v-img></v-avatar>
                            </td>
                            <td>{{ item.id }}</td>
                            <td>{{ item.name }}</td>
                            <td>{{ item.address }}</td>
                            ...
                            <td>
                                <v-btn class="mr-5" color="yellow"
                                    @click="employeeStore.editedEmployee(item)">Edit</v-btn><v-btn color="red"
                                    @click="deleteEmployee(item.id + '')">Delete</v-btn>
                            </td>
                        </tr>
                    </tbody>
                </v-table>
