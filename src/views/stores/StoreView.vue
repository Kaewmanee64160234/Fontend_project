  ...
  <VContainer>
    <VCard>
      <VCardTitle>
        ...
        <VBtn class="mdi mr-2  mdi-delete" style="float: right; color: white" color="red" @click="deleteAllStores">Delete All</VBtn>
        <VSpacer> </VSpacer>
        <VTextField style="width: 20%" variant="solo" color="deep-purple-accent-4" class="mt-7" density="compact"
          append-inner-icon="mdi-magnify" label="Search" single-line hide-details v-model="storeStore.search"></VTextField>
        <VTable class="text-center mt-5">
          ...
          <tbody>
            <tr v-for="item of stores" :key="item.id" class="text-center">
              <td>
                <VCheckbox class="d-flex pa-4" color="indigo" v-model="storeStore.selected" @click="storeStore.selectStore()"
              ...
              <td>{{ item.tel }}</td>
              <td>
                <VBtn color="yellow" class="mr-5 mdi mdi-pencil" @click="storeStore.editStore(item)">Edit</VBtn>
